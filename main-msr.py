...
from models.CLR_Model_finetune import ContrastiveLearningModel
...
def evaluate(model, criterion, data_loader, device, epoch, print_freq, logger):
    batch_time = utils.AverageMeter()
    losses = utils.AverageMeter()
    top1 = utils.AverageMeter()
    top5 = utils.AverageMeter()
...
def main(args):
    ...
    if os.path.isfile(args.finetune):
        ...
        for k in list(state_dict.keys()):
            if not k.startswith(('module.conv')): # ,'module.pos_embedding','module.transformer','module.mlp_head'
                del state_dict[k]

        log = model.load_state_dict(state_dict, strict=False)
        print(log.missing_keys)
        assert log.missing_keys == [
                                    'module.fc_out0.weight', 'module.fc_out0.bias',
                                    'module.fc_out_bn0.weight', 'module.fc_out_bn0.bias',
                                    'module.fc_out_bn0.running_mean',  'module.fc_out_bn0.running_var',
                                    'module.fc_out1.weight', 'module.fc_out1.bias']

        logger.info(("===> Loaded checkpoint with epoch {}".format(checkpoint['epoch'])))
    else:
        logger.info(("===> There is no checkpoint at '{}'".format(args.finetune)))
        sys.exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # if epoch in [0, 5, 6, 10, 11, 13, 15, 17, 19]:
        test_loss, test_top1, test_top5, total_acc = evaluate(model, criterion, val_loader, 
                                                            device, epoch, args.print_freq, logger)
        is_best = total_acc > acc
...
def parse_args():
    ...
    parser.add_argument('--data-path', default='/home/data-vol-2/137data/home/yckj3949/data/MSRAction/processed_data', metavar='DIR', help='path to dataset')
    parser.add_argument('--data-meta', default='/home/data-vol-2/137data/home/yckj3949/data/MSRAction/processed_data', help='dataset')
    ...
    parser.add_argument('--finetune', default='/ml_workspace/yckj3949/code/sxx2/qikan_tuozhan_0322/0425-Point-sequences-unsupervised-dp-global-msr-recontrust-new-base-1012-copy/log_adam_cosine_new_recover_seperate_detach_two_pos_low_level_64token/MSRAction/checkpoint_199.pth', help='finetune from checkpoint')
    ...
    parser.add_argument('--output-dir', default='exp_24frame/batch16_lr0.01_24frames_two_linear_C1_F1_8ka_199epoch_detach_two_pos_lowlevel_64', type=str, help='path where to save')
    parser.add_argument('--log-dir', default='exp_24frame/batch16_lr0.01_24frames_two_linear_C1_F1_8ka_199epoch_detach_two_pos_lowlevel_64', type=str, help='path where to save')
